...
if (isset($data['id_dangky'], $data['total_price'], $data['payment_status'], $data['hinhthuc_thanhtoan'], $data['diadiem'], $data['cart_items'], $data['adminPhone'])) {
    ...
    try {
        ...
        $sql = "INSERT INTO thanhtoan (id_dangky, code_thanhtoan, thanhtoan_status, hinhthuc_thanhtoan, diadiem, gia_thanhtoan, ten_sanpham, soluong, adminPhone) 
                VALUES (:id_dangky, :code_thanhtoan, :thanhtoan_status, :hinhthuc_thanhtoan, :diadiem, :gia_thanhtoan, :ten_sanpham, :soluong_sanpham, :adminPhone)";
        ...
        $stmt->bindParam(':adminPhone', $adminPhone, PDO::PARAM_STR);

        if ($stmt->execute()) {
            $sql = "DELETE FROM cart WHERE id_dangky = :id_dangky";
            $stmt = $conn->prepare($sql);
