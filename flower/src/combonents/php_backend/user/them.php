...
try {
    ...
    if (empty($data['nameUser']) || empty($data['email']) || empty($data['password']) || empty($data['phone']) || !isset($data['admin_status'])) {
    ...
    $sql = "INSERT INTO dangky (name, email, password, phone, admin_status) 
            VALUES ('$nameUser', '$email', '$hashedPassword', '$phone', '$admin_status')";
